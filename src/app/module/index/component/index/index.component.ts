...
import {ElectronService, EventType} from '../../../../service/electron.service';
...
export class IndexComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.electronService.playStatusChange.subscribe(status => {
      switch (status) {
        case 'last':
          this.onPlayStatusChange(PlayEvent.PREVIOUS);
          break;
        case 'play':
          this.musicPlayService.isPlayingNow() ? this.onPlayStatusChange(PlayEvent.PAUSE) : this.onPlayStatusChange(PlayEvent.PLAY);
          break;
        case 'next':
  ...
  private refreshMusicInfo(nowPlayingMusicId: string): void {
    ...
    if (nowPlayMusic) {
      ...
      this.electronService.sendEvent(EventType.TRAY_TOOL_TIP, `${this.nowPlayMusicInfo}-云舒音乐`);
    } else {
      this.nowPlayMusicInfo = '';
      this.titleService.setTitle('云舒音乐');
      this.electronService.sendEvent(EventType.TRAY_TOOL_TIP, '云舒音乐');
    }
  }
